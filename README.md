...
This project is a continuation of the one started during the PC2 where we built a full stack web app **EXPAT$**. [Link here](https://github.com/Geofflac/Expat-app.git) for the Github repository.\
Given the time constrain (1 week), and not having prior experience in this field I use an **AGILE** approached by defining short **SPRINTS** to implement a few features at a time.\
...
<img src="https://user-images.githubusercontent.com/91649093/135460005-eed7ab68-c8ba-4e17-aa32-e77ad3b6b18b.png" width="450"><img src="https://user-images.githubusercontent.com/91649093/135460045-00ab9d50-5bb1-43fa-9998-72240a2fbce1.png" width="450">\
  \
The issues in the GitHub repository can be used too if a **GitHub Project** is open. This project features is a duplicate of the Flying Donut. It will not be used for this project.\
  \
...
It uses the module **CLOUD STORAGE => BROWSER** from Google cloud. It creates a public bucket where the files that are uploaded are made accessible by an URL.\
  \
...
I deploy :
 ...
 - The simplified static version of the EXPAT$ app. [https://storage.googleapis.com/expats_static/index.html](https://storage.googleapis.com/expats_static/index.html).\
...
The first feature is the greeting for new contributor for on the repository, a new issue or pull request triggers this **ACTION** (GitHub module) and reply automatically to the person who generates the request.\
  \
...
## Sprint 4
For this sprint, the target is to deploy a simplified full stack node.js app to the cloud.
To do so, I follow the following tutorial : [Tutorial](https://cloud.google.com/appengine/docs/standard/nodejs/building-app)\
I create a first test with again a hello world app : [https://hello-world-app-327706.as.r.appspot.com/](https://hello-world-app-327706.as.r.appspot.com/)\
I create then a simplified version of the EXPAT$ app : [https://expats-app.as.r.appspot.com/logged](https://expats-app.as.r.appspot.com/logged)\
  \
...
## Sprint 5
For this sprint, the target is to auto deploy the app to Google Cloud by a new commit to the Github Repository. I got most of the informations from this tutorial : [Tutorial](https://medium.com/swlh/deploying-github-repository-to-google-cloud-platform-997d296547e6).\
...
![image](https://user-images.githubusercontent.com/84672157/135811301-be0750b6-12ba-4449-96e0-e6452175d3ae.png)\
  \
 From there, the **GOOGLE CLOUD BUILD** module needs to be activated to trigger an automatic action from a new commit to the App Engine module. For that, a file **cloudbuild.yaml** file is added to the code to run the deployment command automatically.\
  \
...
![image](https://user-images.githubusercontent.com/84672157/135815372-838d52cc-ec41-4bdb-9c40-8fb2b35def7a.png)\
  \
Here is the link for the simplified EXPAT$ app deployed : [https://expats-app-github.as.r.appspot.com/logged](https://expats-app-github.as.r.appspot.com/logged).\
  \
Here is the updated general view :\
